...
class RPM(object):
    ...
    def build_distributions(self):
        ...
        for n in range(partition_size, self.size + 1, partition_size):
            if self.learn_start <= n <= self.priority_size:
                ...
                # set k = batch_size, each segment has total probability is 1 /
                # batch_size
    ...
    def retrieve(self, indices):
        ...
        """
        batch = [self._experience[v] for v in indices]
        res = []
        for i in range(len(self._experience[1])-1): # -1 b/c skip td error
    ...
    def sample(self, global_step):
        ...
        for n in range(1, self.batch_size + 1):
            lower_bound = distribution['strata_ends'][n] + 1
            upper_bound = distribution['strata_ends'][n + 1]
